...
var terrain_resource: TerrainRes
...
func load_terrain():
	terrain_resource = ResourceLoader.load(save_path + save_file_name)
	if terrain_resource:
		terrain_resource = terrain_resource.duplicate(true)
...
func partly_load():
	...
	if terrain_resource:
		...
		terrain_resource = TerrainRes.new()
	if array_layers__dict_ground_pos___id__left.size() < 5:
...
#region save load
func save_array_layers_and_all(_array_layers__dict_ground_pos___id__left):
	array_layers__dict_ground_pos___id__left = _array_layers__dict_ground_pos___id__left
...
func save_position__array_data_array(position: Vector2i, layer: int, array_data_array: Array):
	if array_layers__dict_ground_pos___id__left.size() < 5:
		array_layers__dict_ground_pos___id__left.resize(5)
		array_layers__dict_ground_pos___id__left.fill({})
	array_layers__dict_ground_pos___id__left[layer].set(position, array_data_array)
	#var positions = array_layers__dict_ground_pos___id__left[layer].set(position, array_data_array)
	#var _array_data_array = positions.get_or_add(position, array_data_array)
	terrain_resource.array_layers__dict_ground_pos___id__left = array_layers__dict_ground_pos___id__left
	#verify_save_dir()
	save_terrain()
