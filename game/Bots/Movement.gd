...
func _physics_process(delta: float) -> void:
	...
	var speed = RUN_SPEED * relative_speed
	if state == State.WALK or state == State.BROKEN:
		speed = WALK_SPEED * relative_speed
	...
	if not is_zero_approx(body.velocity.x):
		...
		if body.velocity.y < 0.0:
			...
			visuals.scale.x = -1.0 * sprite_scale
	# it is overriden with controller
	#body.move_and_slide()
	...
	if abs(body.velocity.x) > 50 or abs(body.velocity.y) > 50:
		...
		animation_tree["parameters/walk_timescale/scale"] = time_scale / (12 / relative_speed)
	else:
